...
export default {
  ...
  methods : {
    onInit(){
      this.modal_product = new bootstrap.Modal(document.getElementById('modal-product-data'), {
      	keyboard: false
		  })

      this.modal_product.show();
    ...
    onLoad(){
      ...
      this.$axios.get("/transaction/product/get-product",{
    ...
    onChooseItem(item){
      this.modal_product.hide();
      
      this.onChooseProduct(item); 
    }
  }
}
</script>
