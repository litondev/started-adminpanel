...
export default {
  ...
  methods : {
    onInit(){
      this.modal_customer = new bootstrap.Modal(document.getElementById('modal-customer-data'), {
  		  keyboard: false
		  })

      this.modal_customer.show();
    ...
    onLoad(){
      ...
      this.$axios.get("/transaction/product/get-customer",{
    ...
    onChooseItem(item){
      this.modal_customer.hide();
      
      this.onChooseCustomer(item); 
    }
  }
}
</script>
