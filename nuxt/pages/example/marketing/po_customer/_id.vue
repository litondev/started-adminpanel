                    ...
                    <form @submit.prevent="validate().then(onSubmit(invalid))"
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="form-group">
                                    <label for="supplier_id">Penawaran</label>
                                       <v-select                           
                                        label="code"   
                                        ...
                                        <li slot-scope="{search}" slot="list-footer"
                                            ...
                                            <span v-if="lookup_quotations.current_page > 1" 
                                                @click="onGetQuotation(search,false)"
                                                class="flex-fill bg-primary text-white text-center"
                                                style="cursor:pointer">Sebelumnya</span>
                                            <span v-if="lookup_quotations.last_page > lookup_quotations.current_page" 
                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table table-sm mt-3">
                                ...
                                <tr v-for="item,index in form.po_customer_details"
                                    :key="index">
                                    <td>
                                        <input type="text" 
                                            class="form-control"
...
export default {
    ...
    async created(){
        try{        
            if(this.isEditable){
                let response = await this.$axios.get("marketing/po_customer/"+this.id);
                ...
                this.form.po_customer_details = response.data
                    .po_customer_details
                    .map(item => {
                        return {
                            ...item,
    ...
    methods: {    
        ...
        deleteDetail(indexItem){
            this.form.po_customer_details = this.form
                .po_customer_details
        ...
        onSubmit(isInvalid){
            ...
            let formData = {
                ...
                customer_id : typeof this.form.customer_id == 'object'
                    ...
                    : '',
                quotation_id : typeof this.form.quotation_id == 'object'
                    ? this.form.quotation_id.id 
                    : ''
            }

            formData.po_customer_details = formData
                .po_customer_details
                .map(item => {
                    return {
        ...
        itemChange(index){
            let po_customer = this.form.po_customer_details[index];

            if(
                po_customer.quantity >= 0.00
                &&
                po_customer.price >= 0.00
            ){
                let amount =  po_customer.quantity * po_customer.price    

                po_customer.amount = amount 
        ...
        onSelectQuotation(item){
            if(item){
                this.form = {
                    ...
                    po_customer_details : item.quotation_details
                        .map(item => {
                            return {
                                product_id :  item.product,
                                quantity : item.quantity,
        ...
        onSetProduct(index){
            let product = this.form.po_customer_details[index];
